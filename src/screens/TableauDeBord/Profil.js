...
export default function Profil({ navigation }) {
  ...
  useEffect(() => {
    ...
    const hydrateFromRedux = () => {
      if (!reduxUser || !mounted) return;
      setUser((prev) => ({
        ...prev,
        name:
          reduxUser.full_name ||
          reduxUser.name ||
          (reduxUser.email ? reduxUser.email.split("@")[0] : "Utilisateur"),
        email: reduxUser.email || "",
        phone: reduxUser.phone || "",
        campus: reduxUser.campus || prev.campus,
        role: reduxUser.role || "user",
        createdPaths: Number.isFinite(reduxUser.created_paths)
          ? reduxUser.created_paths
          : prev.createdPaths,
        savedPaths: Number.isFinite(reduxUser.saved_paths)
          ? reduxUser.saved_paths
          : prev.savedPaths,
      }));
    };
