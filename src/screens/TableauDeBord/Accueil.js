...
// src/screens/TableauDeBord/Accueil.js
...
export default function Accueil({ navigation }) {
  ...
  // ✅ Stopper toutes les vidéos quand on quitte l'écran
  useFocusEffect(
    useCallback(() => {
      loadUser();
      setCurrentIndex(0); // reprendre la première vidéo au retour
      return () => {
        setCurrentIndex(-1); // ✅ stoppe toutes les vidéos en quittant
      };
    }, [loadUser])
  );
  ...
  const handleViewMap = (path) => {
    ...
    if (validCoords.length < 1) {
      Alert.alert('Carte indisponible', "Ce chemin n'a pas encore de coordonnées GPS.", [{ text: 'OK' }]);
      return;
    }
...
function VideoItem({ path, isActive, onToggleFavorite, onViewSteps, onViewMap, onShare }) {
  ...
  // ✅ Play/Pause + mute selon isActive
  useEffect(() => {
    if (!player) return;
    try {
      if (isActive && videoSource) {
        ...
        // ✅ Stopper le son ET la vidéo quand inactif ou hors focus
        player.pause();
        player.muted = true;
        setIsPlaying(false);
      }
  ...
  const handleDownload = async () => {
    ...
    if (!rawUrl) { Alert.alert('Indisponible', 'Aucune vidéo disponible.'); return; }
    try {
      ...
      const { status: httpStatus, uri: downloadedUri } = await FileSystem.downloadAsync(downloadUrl, localUri);
      if (httpStatus !== 200) throw new Error(`Erreur serveur (${httpStatus})`);
      const info = await FileSystem.getInfoAsync(downloadedUri);
      if (!info.exists || info.size === 0) throw new Error('Fichier vide');
  ...
  return (
    <View style={styles.videoContainer}>
      ...
      {/* Zone de tap */}
      <TouchableOpacity style={styles.tapZone} activeOpacity={1} onPress={showControlsTemporarily} />
      ...
      <View style={styles.videoActionsContainer}>
        ...
        <TouchableOpacity style={styles.videoActionButton} onPress={handleDownload} activeOpacity={0.7} disabled={downloading}>
          {downloading ? (
            <ActivityIndicator size="small" color="#FEBD00" style={{ width: 32, height: 32 }} />
          ) : (
            <Ionicons name="download-outline" size={32} color="#FEBD00" />
          )}
          <Text style={[styles.videoActionText, { color: '#FEBD00' }]}>{downloading ? '...' : 'Sauver'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  tapZone: { position: 'absolute', top: 80, left: 0, right: 100, bottom: 200 },
  videoControls: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.25)' },
  controlBtn: { alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 40, width: 56, height: 56, gap: 2 },
  controlBtnMain: { width: 72, height: 72, backgroundColor: 'rgba(254,189,0,0.9)', borderRadius: 36 },
  controlBtnText: { color: '#fff', fontSize: 10, fontWeight: 'bold' },
