...
export default function StepCreationScreen({ route, navigation }) {
  // ✅ coordinates ajouté dans les params reçus
  const {
    videoUri,
    videoDuration,
    departure,
  ...
  const removeStep = (index) => {
    ...
    const updatedSteps = steps.filter((_, i) => i !== index);
    updatedSteps.forEach((step, i) => { step.step_number = i + 1; });
    setSteps(updatedSteps);
    if (currentStepIndex >= updatedSteps.length) {
      setCurrentStepIndex(updatedSteps.length - 1);
    }
  ...
  const handleSubmit = () => {
    ...
    const invalidSteps = steps.filter(step => step.start_time >= step.end_time);
    if (invalidSteps.length > 0) {
      Alert.alert('Timings invalides', 'Chaque étape doit avoir un temps de début inférieur au temps de fin');
      return;
    }

    // ✅ coordinates transmis à PathConfirmation
    navigation.navigate('PathConfirmation', {
      videoUri,
      videoDuration,
      departure,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.videoContainer}>
        ...
        <View style={styles.videoInfo}>
          <Text style={styles.videoTime}>{formatTime(currentTime)} / {formatTime(videoDuration)}</Text>
        </View>
      </View>
      ...
      <View style={styles.gpsBar}>
        <Ionicons
          name={coordinates.length > 0 ? 'navigate' : 'navigate-outline'}
          size={16}
          color={coordinates.length > 0 ? '#34C759' : '#999'}
        />
        <Text style={[styles.gpsBarText, { color: coordinates.length > 0 ? '#34C759' : '#999' }]}>
          {coordinates.length > 0
            ? `${coordinates.length} points GPS enregistrés ✅`
            : 'GPS non capturé (vidéo depuis galerie)'}
        </Text>
      </View>
      ...
      <ScrollView style={styles.stepsContainer} showsVerticalScrollIndicator={false}>
        {steps.map((step, index) => (
          <View key={index} style={[styles.stepCard, currentStepIndex === index && styles.stepCardActive]}>
            <View style={styles.stepHeader}>
              <TouchableOpacity
                style={styles.stepNumberButton}
                onPress={() => { setCurrentStepIndex(index); goToStep(step); }}
              >
                <LinearGradient
                  colors={currentStepIndex === index ? ['#FEBD00', '#FFD700'] : ['#e0e0e0', '#e0e0e0']}
                  style={styles.stepNumberGradient}
              ...
              <View style={styles.timeInputGroup}>
                ...
                <View style={styles.timeInputContainer}>
                  <TextInput
                    style={styles.timeInput}
                    value={step.start_time.toString()}
                    onChangeText={(text) => updateStepTime(index, 'start_time', parseInt(text) || 0)}
                    keyboardType="number-pad"
                    maxLength={3}
                  />
                  <Text style={styles.timeUnit}>sec</Text>
              ...
              <View style={styles.timeInputGroup}>
                ...
                <View style={styles.timeInputContainer}>
                  <TextInput
                    style={styles.timeInput}
                    value={step.end_time.toString()}
                    onChangeText={(text) => updateStepTime(index, 'end_time', parseInt(text) || 0)}
                    keyboardType="number-pad"
                    maxLength={3}
                  />
                  <Text style={styles.timeUnit}>sec</Text>
            ...
            <View style={styles.instructionSection}>
              ...
              <TextInput
                ...
                maxLength={200}
              />
              <Text style={styles.charCount}>{step.text.length}/200</Text>
            </View>

            <TouchableOpacity style={styles.previewStepButton} onPress={() => goToStep(step)}>
              <Ionicons name="play-circle" size={20} color="#FEBD00" />
              <Text style={styles.previewStepText}>Voir cette étape</Text>
            </TouchableOpacity>
          </View>
        ))}
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
          <LinearGradient colors={['#FEBD00', '#FFD700']} style={styles.submitGradient}>
...
const styles = StyleSheet.create({
  ...
  stepCounter: { backgroundColor: 'rgba(255,255,255,0.3)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  stepCounterText: { fontSize: 14, fontWeight: 'bold', color: '#333' },
  videoContainer: { height: height * 0.25, backgroundColor: '#000', position: 'relative' },
  video: { width: '100%', height: '100%' },
  playButton: { position: 'absolute', top: '50%', left: '50%', marginLeft: -30, marginTop: -30, width: 60, height: 60, borderRadius: 30, backgroundColor: 'rgba(254,189,0,0.9)', justifyContent: 'center', alignItems: 'center' },
  videoInfo: { position: 'absolute', bottom: 10, right: 10, backgroundColor: 'rgba(0,0,0,0.7)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  videoTime: { color: '#fff', fontSize: 12, fontWeight: 'bold' },
  gpsBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f0f0f0', paddingHorizontal: 16, paddingVertical: 8, gap: 6 },
  gpsBarText: { fontSize: 12, fontWeight: '500' },
  instructionsCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF4D6', marginHorizontal: 20, marginTop: 12, padding: 12, borderRadius: 12, gap: 8 },
  instructionsText: { flex: 1, fontSize: 13, color: '#666' },
  stepsContainer: { flex: 1, paddingHorizontal: 20, paddingTop: 16 },
  stepCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 2, borderColor: 'transparent' },
  stepCardActive: { borderColor: '#FEBD00', backgroundColor: '#FFFBF0' },
  stepHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
  stepNumberButton: { marginRight: 12 },
  stepNumberGradient: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center' },
  stepNumber: { fontSize: 18, fontWeight: 'bold', color: '#fff' },
  stepTitle: { flex: 1, fontSize: 16, fontWeight: 'bold', color: '#333' },
  deleteButton: { padding: 8 },
  timingSection: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 },
  timeInputGroup: { flex: 1 },
  timeLabel: { fontSize: 12, color: '#999', marginBottom: 4, fontWeight: '600' },
  timeInputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f5f5f5', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8 },
  timeInput: { flex: 1, fontSize: 16, fontWeight: 'bold', color: '#333' },
  timeUnit: { fontSize: 12, color: '#999', marginLeft: 4 },
  sliderSection: { marginBottom: 12 },
  sliderLabel: { fontSize: 12, color: '#666', marginBottom: 4 },
  slider: { width: '100%', height: 40 },
  instructionSection: { marginTop: 8 },
  instructionLabel: { fontSize: 14, fontWeight: 'bold', color: '#333', marginBottom: 8 },
  instructionInput: { backgroundColor: '#f5f5f5', borderRadius: 8, padding: 12, fontSize: 14, color: '#333', minHeight: 80, textAlignVertical: 'top' },
  charCount: { fontSize: 11, color: '#999', textAlign: 'right', marginTop: 4 },
  previewStepButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 12, paddingVertical: 10, gap: 6 },
  previewStepText: { fontSize: 14, fontWeight: '600', color: '#FEBD00' },
  addStepButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', borderRadius: 12, borderWidth: 2, borderColor: '#FEBD00', borderStyle: 'dashed', padding: 16, marginBottom: 16, gap: 8 },
  addStepText: { fontSize: 16, fontWeight: 'bold', color: '#FEBD00' },
