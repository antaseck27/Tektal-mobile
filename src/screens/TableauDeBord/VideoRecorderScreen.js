...
// /Users/antayussuf/Desktop/volkeno/tektal-mobile/src/screens/TableauDeBord/VideoRecorderScreen.js
import React, { useState, useRef, useEffect } from "react";
import {
  View,
  Text,
...
export default function VideoRecorderScreen({ route, navigation }) {
  ...
  const handleStartRecordingStep = () => {
    if (!description.trim()) {
      Alert.alert("Description manquante", "Veuillez ajouter une description du trajet.");
      return;
    }
  ...
  const buildDefaultSteps = (totalSeconds) => {
    const total = Math.min(Math.max(totalSeconds || 2, 2), 45);
    const mid = Math.max(1, Math.floor(total / 2));
    return [
      { title: "Départ", start_time: 0, end_time: mid },
      { title: "Arrivée", start_time: mid, end_time: total },
    ];
  };
  ...
  const handleValidate = async () => {
    ...
    if (!departure || !destination) {
      Alert.alert("Erreur", "Point de départ et destination obligatoires");
      return;
    }
    ...
    try {
      ...
      // 1) Upload vidéo vers Cloudinary
      const cloudName = "dxkadqzzz";
      const uploadPreset = "tektal_unsigned"; // <-- remplace par ton vrai preset unsigned

      const uploadForm = new FormData();
      uploadForm.append("file", {
        uri: videoUri,
        type: "video/mp4",
        name: `tektal_${Date.now()}.mp4`,
      });
      uploadForm.append("upload_preset", uploadPreset);
      uploadForm.append("folder", "tektal/paths");
      ...
      if (!uploadRes.ok || !uploadData?.secure_url) {
        Alert.alert(
          "Erreur upload",
          uploadData?.error?.message || "Impossible d'uploader la vidéo"
        );
        return;
      }

      const videoUrl = uploadData.secure_url;
      const durationSeconds = Math.min(Math.max(recordingTime || 1, 1), 45);

      // 2) Payload conforme serializer backend PathCreateSerializer
      const payload = {
        title: `${departure} -> ${destination}`,
        start_label: departure,
        end_label: destination,
        start_lat: coordinates?.[0]?.latitude ?? null,
        start_lng: coordinates?.[0]?.longitude ?? null,
        end_lat: coordinates?.[coordinates.length - 1]?.latitude ?? null,
        end_lng: coordinates?.[coordinates.length - 1]?.longitude ?? null,
        video_url: videoUrl,
        duration: durationSeconds,
        is_official: pathType === "official",
        status: "published",
        steps: buildDefaultSteps(durationSeconds),
      };
      ...
      if (!res.ok) {
        Alert.alert(
          "Erreur publication",
          data?.detail || JSON.stringify(data) || "Impossible de publier le chemin"
        );
        return;
      }

      Alert.alert("Succès", "Chemin publié avec succès", [
        {
          text: "OK",
          onPress: () => {
            navigation.reset({
      ...
      Alert.alert("Erreur", "Une erreur est survenue pendant la publication");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (!permissionsReady) {
    return (
      <View style={styles.permissionContainer}>
        ...
        <Text style={styles.permissionSubtext}>
          Autorisez la caméra, le micro et la localisation GPS.
        </Text>
  ...
  if (step === "description") {
    return (
      <KeyboardAvoidingView
        ...
        behavior={Platform.OS === "ios" ? "padding" : "height"}
      >
        <LinearGradient colors={["#FEBD00", "#FFD700"]} style={styles.descriptionScreenGradient}>
          <ScrollView
            contentContainerStyle={styles.descriptionScreenContent}
            keyboardShouldPersistTaps="handled"
          >
            <View style={styles.descriptionScreenHeader}>
              <TouchableOpacity style={styles.closeButtonWhite} onPress={() => navigation.goBack()}>
                <Ionicons name="close" size={30} color="#333" />
              </TouchableOpacity>
            </View>
            ...
            <Text style={styles.descriptionScreenTitle}>Décrivez votre trajet</Text>
            <Text style={styles.descriptionScreenSubtitle}>
              {departure} -> {destination}
            </Text>

            <View style={styles.descriptionCard}>
              <Text style={styles.descriptionLabel}>Description du trajet</Text>
              <TextInput
                style={styles.descriptionInput}
                placeholder="Ex: Passez par la grande porte, tournez à droite..."
                placeholderTextColor="#999"
                value={description}
                onChangeText={setDescription}
                multiline
            ...
            <View style={styles.tipsCard}>
              ...
              <Text style={styles.tipsText}>
                • Décrivez les points de repère visibles{"\n"}• Mentionnez les virages{"\n"}• Indiquez les obstacles éventuels
              </Text>
            </View>

            <TouchableOpacity
              style={[styles.nextButton, !description.trim() && styles.nextButtonDisabled]}
              onPress={handleStartRecordingStep}
              disabled={!description.trim()}
            >
              <Text style={styles.nextButtonText}>Commencer l'enregistrement</Text>
              <Ionicons name="arrow-forward" size={24} color="#fff" />
            </TouchableOpacity>
          </ScrollView>
        </LinearGradient>
    ...
    return (
      <View style={styles.container}>
        <CameraView ref={cameraRef} style={styles.camera} facing={facing} mode="video">
          <View style={styles.overlay}>
            <View style={styles.header}>
              <TouchableOpacity style={styles.closeButton} onPress={() => setStep("description")}>
                <Ionicons name="arrow-back" size={30} color="#fff" />
              </TouchableOpacity>
            ...
            {!isRecording && (
              <View style={styles.instructionsContainer}>
                ...
                <Text style={styles.instructionsSubtext}>Le GPS enregistre automatiquement votre parcours</Text>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  descriptionScreenGradient: { flex: 1 },
  descriptionScreenContent: {
    flexGrow: 1,
    paddingHorizontal: 20,
    paddingBottom: 40,
  ...
  tipsText: { fontSize: 13, color: "#666", lineHeight: 20 },
  nextButton: {
    backgroundColor: "#333",
    flexDirection: "row",
    justifyContent: "center",
  ...
  backButton: { backgroundColor: "#FEBD00", paddingHorizontal: 30, paddingVertical: 14, borderRadius: 25 },
  ...
  savingText: { marginTop: 15, fontSize: 16, fontWeight: "bold", color: "#333" },
});
