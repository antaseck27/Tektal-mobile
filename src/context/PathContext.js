...
const resolveCreatorName = async (userObj) => {
  if (!userObj) return 'Inconnu';
  if (userObj.full_name?.trim()) return userObj.full_name.trim();
  if (userObj.name?.trim()) return userObj.name.trim();
  ...
  if (userObj.id) {
    try {
      ...
      if (res.ok && res.data) {
        const u = res.data;
        return u.full_name?.trim() || u.name?.trim() || (u.email ? u.email.split('@')[0] : 'Inconnu');
      }
    } catch {}
  }
  return 'Inconnu';
...
export const PathProvider = ({ children }) => {
    ...
    try {
      ...
      if (favoritesResult.ok && favoritesResult.data) {
        ...
        favArr.forEach((fav) => {
          ...
          if (pathId != null && !isNaN(Number(pathId))) favoriteIds.add(Number(pathId));
        });
        ...
        const formattedPaths = await Promise.all(
          pathsArray.map(async (path) => {
            ...
            if (coordinates.length === 0) {
              const startLat = parseFloat(path.start_lat);
              const startLng = parseFloat(path.start_lng);
              const endLat = parseFloat(path.end_lat);
              const endLng = parseFloat(path.end_lng);
              const startValid = !isNaN(startLat) && !isNaN(startLng);
              const endValid = !isNaN(endLat) && !isNaN(endLng);
            ...
            const creatorName = await resolveCreatorName(path.user);
            const establishmentName = path.establishment?.name || path.establishment_name || null;
            ...
            return {
              ...
              campus: establishmentName || 'Tektal',
  ...
  const toggleFavorite = async (pathId) => {
    try {
      ...
      console.log(isCurrentlyFavorite ? '💔 Favori supprimé via API:' : '❤️ Favori ajouté via API:', result);
